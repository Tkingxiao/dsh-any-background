  ...
  `;
		...
		*  can re-sync the settings preview / store). */
		...
		const OPACITY_TOKEN_GROUPS = [
			...
			{
				part: "input",
				names: ["--dsw-specific-input-major", "--dsw-specific-menu"]
			}
		];
		const OPACITY_VARS = {
		...
		const INPUT_BLUR_RULE = "[data-composer-card],[data-cordis-panel]{-webkit-backdrop-filter:var(--dsh-any-input-blur,none);backdrop-filter:var(--dsh-any-input-blur,none)}";
