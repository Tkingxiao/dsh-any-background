window.__ModuleLoader__.load({
		...
		function createMeshGradient(params, canvas) {
			...
			const render = () => {
				...
				for (const b of blobs) {
					...
					const cy = ((b.y + noise(b.hx, b.hy + phase * .3) * .25 + phase * .015) % 1 + 1) % 1 * h;
					...
					rad.addColorStop(.55, `hsla(${b.hue},${Math.round(b.sat * .6)}%,${b.lit}%,${(+alpha * .35).toFixed(3)})`);
  ...
  `;
		...
		function createDots(params, canvas) {
			...
			const dark = rng() < .5;
			const baseHue = Math.round(rng() * 360);
			const spacing = Math.max(24, 140 * params.scale / (.25 + params.density));
			const dots = [];
			...
			for (let y = 0; y < rows; y++) for (let x = 0; x < cols; x++) dots.push({
				bx: (x + .5) * spacing,
				by: (y + .5) * spacing,
				r: spacing * .18 * params.scale * (.6 + rng() * .7),
			...
			const render = () => {
				...
				for (const d of dots) {
					...
					const x = d.bx * dpr;
					const y = d.by * dpr + Math.sin(t * d.speed * .5 + d.phase) * 4 * dpr;
					g.beginPath();
		...
		function createLowPoly(params, canvas) {
			...
			for (let y = 0; y < rows; y++) for (let x = 0; x < cols; x++) cells.push({
				...
				lit: dark ? Math.round(12 + rng() * 35) : Math.round(65 + rng() * 25),
				phase: rng() * Math.PI * 2
			});
			let t = 0;
			let running = true;
			const render = () => {
				...
				for (const cell of cells) {
					...
					const litShift = Math.sin(t * .4 + cell.phase) * 6;
