	...
	factory: (require) => {
		...
		*  call sites pass this so the palette and the scheme flag never diverge. */
		function rScheme() {
			...
			if (o !== "auto") return o;
			return rBgDark() ?? (rHasColor() ? rColor()[2] < .55 : false) ? "dark" : "light";
		}
		...
		function apply(ctx) {
			...
			const registerCustom = (h, s, l) => {
				...
				try {
					let colorScheme;
					let tokens;
					if (rHasColor()) ({colorScheme, tokens} = genTokens(h ?? rColor()[0], s ?? rColor()[1], l ?? rColor()[2], rScheme()));
