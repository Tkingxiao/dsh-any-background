...
└── dsh-any-background/       # The plugin package
    ...
    │   └── client/
    │       └── index.tsx     # Browser half — ALL UI logic lives here (~886 lines)
    └── lib/                  # Built output (generated by tsdown)
        ├── index.js          # Node entry
...
Clone the DeepSeek Harness repository and build from source:
...
# 3. Create a plugin folder and copy the plugin into it
mkdir -p plugins
...
# 5. Install the plugin into the web profile
